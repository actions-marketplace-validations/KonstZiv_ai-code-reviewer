...
Для **всіх планів GitLab** (включаючи Free). Рекомендовано для локального запуску або повної функціональності в CI.
...
2. Додайте змінну:
    - **Key:** `AI_REVIEWER_GITLAB_TOKEN` (або `GITLAB_TOKEN`)
    - **Value:** вставте ваш токен
    - **Flags:** увімкніть **Masked** та **Protected**
3. Використовуйте в `.gitlab-ci.yml`:

```yaml
variables:
  AI_REVIEWER_GITLAB_TOKEN: $GITLAB_TOKEN  # Personal Access Token з CI/CD Variables
```
...
```yaml
variables:
  AI_REVIEWER_GITLAB_TOKEN: $GITLAB_PROJECT_TOKEN  # Project Access Token з CI/CD Variables
```

!!! info "Який токен обрати?"
    | | CI_JOB_TOKEN | Personal Access Token | Project Access Token |
    |---|---|---|---|
    | **План** | Всі | Всі (включаючи Free) | Лише Premium/Ultimate |
    | **Налаштування** | Автоматичне | Ручне | Ручне |
    | **Область дії** | Лише поточний job | Всі проєкти користувача | Один проєкт |
    | **Inline коментарі** | :x: | :white_check_mark: | :white_check_mark: |
    | **Найкраще для** | Швидкий старт | Free план + повні можливості | Команди на Premium/Ultimate |
...
ai-review:
  ...
  variables:
    ...
    AI_REVIEWER_GITLAB_TOKEN: $CI_JOB_TOKEN  # Автоматичний, не потребує налаштування
```
...
ai-review:
  ...
  variables:
    ...
    AI_REVIEWER_GITLAB_TOKEN: $CI_JOB_TOKEN    # або: $GITLAB_PAT (див. "Отримання токена")
...
1. `AI_REVIEWER_GOOGLE_API_KEY` (або `GOOGLE_API_KEY`) змінна встановлена
2. `AI_REVIEWER_GITLAB_TOKEN` (або `GITLAB_TOKEN`) має достатньо прав (scope: `api`)
...
```yaml
variables:
  AI_REVIEWER_GITLAB_TOKEN: $GITLAB_TOKEN  # PAT, не CI_JOB_TOKEN
```
